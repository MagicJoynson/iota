...
The app, so you can point him at things: Ring (home) → tap an arc for University (violet: Today · Modules · Deadlines · Societies — each module and each society opens its own hub; MMU Karting lives under Societies with Club / My Racing / Events), Work (teal: Shifts · Earnings with the pay-by-payday chart · Time off with ask-by reminders · Info) or Personal (orange, the whole-life layer: Week = the unified calendar with a 14-day capacity strip and a Right Now card · Money · Admin = watches and renewals · Targets = life-balance chart). Next Up strip → Personal › Week. Settings (gear on the Ring) has Base switch (Sheffield summer / Manchester term), pay, travel, API key, usage. Hold the orb anywhere for quick capture.
...
Japanese: Alex is learning Japanese for a year abroad in Japan (Sept 2027) — module JPN in University › Modules with the SRS. You can speak Japanese with him, but ONLY at his level: use grammar patterns and words the context marks as known, plus kana; put romaji in brackets after Japanese lines; correct gently in [brackets]; never mock a mistake; switch to English the moment he asks. If reviews are piling up (40+ due) or the streak is at risk, say so once, briefly.`;
  ...
  const TOOLS = [
    ...
    { name: 'add_event', description: 'Add a calendar event (lecture, seminar, karting practice, social, personal appointment).', input_schema: { type: 'object', properties: { kind: { type: 'string', enum: ['uni', 'work', 'kart', 'personal'] }, title: { type: 'string' }, starts_at: { type: 'string' }, ends_at: { type: 'string' }, location: { type: 'string' }, module_id: { type: 'string', description: 'for uni sessions, the module id' }, notes: { type: 'string' } }, required: ['kind', 'title', 'starts_at'] } },
    ...
    { name: 'add_note', description: 'Save a note (kit lists, things to remember, track debriefs). For uni notes pass module_id so it lands in that module\'s hub.', input_schema: { type: 'object', properties: { section: { type: 'string', enum: ['uni', 'work', 'kart', 'personal'] }, module_id: { type: 'string', description: 'id from the Modules list, optional' }, title: { type: 'string' }, md: { type: 'string' } }, required: ['md'] } },
    { name: 'add_module', description: 'Create a module hub. kind=course for an MMU module (from the timetable), kind=personal for a self-directed project (a language, a skill) — e.g. "Japanese for a year abroad". Each gets its own page with notes, sessions and assessments/goals.', input_schema: { type: 'object', properties: { name: { type: 'string' }, code: { type: 'string', description: 'short code, optional (e.g. JPN, BUS101)' }, kind: { type: 'string', enum: ['course', 'personal'] }, lecturer: { type: 'string' }, credits: { type: 'number' }, notes: { type: 'string', description: 'what it is for / why' } }, required: ['name', 'kind'] } },
    { name: 'set_module_grade', description: 'Record the final mark (%) for a module — past or current. Use when Alex reads out results. Also computes nothing itself; report the year average yourself from the Modules list if all marks in a year are present.', input_schema: { type: 'object', properties: { module_id: { type: 'string' }, mark: { type: 'number', description: '0–100; null clears' } }, required: ['module_id', 'mark'] } },
    { name: 'set_assessment_mark', description: 'Record the mark (%) for one assessment by id.', input_schema: { type: 'object', properties: { assessment_id: { type: 'string' }, mark: { type: 'number' } }, required: ['assessment_id', 'mark'] } },
    { name: 'add_module_review', description: 'Add a What-Went-Well (www) or Even-Better-If (ebi) reflection to a module (from a debrief chat).', input_schema: { type: 'object', properties: { module_id: { type: 'string' }, kind: { type: 'string', enum: ['www', 'ebi'] }, text: { type: 'string' }, draft: { type: 'boolean' } }, required: ['module_id', 'kind', 'text'] } },
    { name: 'confirm_module_review', description: 'Confirm (draft→final), edit the text of, or delete a module review by id.', input_schema: { type: 'object', properties: { review_id: { type: 'string' }, action: { type: 'string', enum: ['confirm', 'edit', 'delete'] }, text: { type: 'string' } }, required: ['review_id', 'action'] } },
    { name: 'jp_stats', description: 'Japanese module: due reviews, streak, plan week/stage, pace, accuracy, leeches, days to Japan, and what he knows. Use for "how is my Japanese going?".', input_schema: { type: 'object', properties: {} } },
    { name: 'jp_start_speaking', description: 'Start a Japanese speaking scenario: returns the scenario card + a handful of phrasebook lines to build the role-play from. scenario ∈ greetings|konbini_shopping|restaurant|directions_transport|social|dorm_daily|phone_admin|emergency_health|karting|weather_smalltalk|classroom|numbers_money|free.', input_schema: { type: 'object', properties: { scenario: { type: 'string' } }, required: ['scenario'] } },
  ...
  function contextBlock() {
    ...
    if (s.studentFinance?.drops?.length) lines.push(`Student loan (${s.studentFinance.provider || 'SFE'}, ${s.studentFinance.kind || 'maintenance'}${s.studentFinance.year ? ' ' + s.studentFinance.year : ''}): ` + s.studentFinance.drops.map(d => `£${(+d.amount).toFixed(2)} on ${d.date} (${String(d.status || '').replace('_', ' ')})`).join(', ') + `. Alex calls it "student loan" — don't say SFE unprompted. Update via set_setting studentFinance if he tells you a status changed.`);
    ...
    const mods = Store.list('modules'); const curM = mods.filter(m => m.status !== 'completed'), pastM = mods.filter(m => m.status === 'completed');
    if (curM.length) lines.push('Current modules — id | code | name | kind: ' + curM.map(m => `${m.id} | ${m.code || '-'} | ${m.name} | ${m.kind || 'course'}`).join('; ') + '. Use module_id on add_note/add_event for these.');
    if (pastM.length) { const missing = pastM.filter(m => m.final_mark == null).length; const drafts = Store.list('module_reviews').filter(r => r.draft).length; lines.push(`Past modules (archive): ${pastM.length} completed (${pastM.filter(m => m.year_label === 'Foundation Year').length} Foundation, ${pastM.filter(m => m.year_label === 'Year 1').length} Year 1); grades missing for ${missing}${drafts ? `; ${drafts} draft WWW/EBI reflections awaiting his confirmation` : ''}. If he mentions results or a transcript, offer ONCE to file the marks (set_module_grade / set_assessment_mark) and then work out the Year 1 average — ask for credit weightings if unequal. Ids — id | name | year | mark: ` + pastM.map(m => `${m.id} | ${m.name} | ${m.year_label}${m.semester ? ' S' + m.semester : ''} | ${m.final_mark ?? '-'}`).join('; ')); }
    try { const jl = window.JP && JP.ready ? JP.summary() : null; if (jl) lines.push(jl); else if (Store.list('modules').some(m => m.kind === 'language')) lines.push('Japanese (module JPN) exists; stats load once he opens it (or call jp_stats).'); } catch (_) {}
    const ths = Store.list('theories'); if (ths.length) lines.push('Theory carry-forward (frameworks he already knows → where they return): ' + ths.map(t => `${t.name} → ${(t.returns_in || []).join('/')}`).join('; ') + '.');
  ...
  function runTool(name, input) {
    ...
    switch (name) {
      ...
      case 'add_event': { const r = Store.insert('events', { kind: input.kind, title: input.title, starts_at: new Date(input.starts_at).toISOString(), ends_at: input.ends_at ? new Date(input.ends_at).toISOString() : new Date(new Date(input.starts_at).getTime() + 3600000).toISOString(), location: input.location || null, module_id: input.module_id || null, notes: input.notes || null, status: 'planned', source: 'claude' }); return { ok: true, id: r.id }; }
      ...
      case 'add_note': { const r = Store.insert('notes', { section: input.module_id ? 'uni' : (input.section || 'personal'), module_id: input.module_id || null, title: input.title || null, md: input.md, tags: [] }); return { ok: true, id: r.id }; }
      case 'set_module_grade': { const m = Store.get('modules', input.module_id); if (!m) return { ok: false, error: 'no such module' }; Store.update('modules', m.id, { final_mark: input.mark == null ? null : +input.mark }); return { ok: true, module: m.name }; }
      case 'set_assessment_mark': { const a = Store.get('assessments', input.assessment_id); if (!a) return { ok: false, error: 'no such assessment' }; Store.update('assessments', a.id, { mark: +input.mark, status: 'graded' }); return { ok: true }; }
      case 'add_module_review': { const m = Store.get('modules', input.module_id); if (!m) return { ok: false, error: 'no such module' }; const r = Store.insert('module_reviews', { module_id: m.id, kind: input.kind, text: input.text, draft: !!input.draft }); return { ok: true, id: r.id }; }
      case 'confirm_module_review': { const r = Store.get('module_reviews', input.review_id); if (!r) return { ok: false, error: 'no such review' }; if (input.action === 'delete') { Store.remove('module_reviews', r.id); return { ok: true }; } Store.update('module_reviews', r.id, input.action === 'edit' ? { text: input.text || r.text, draft: false } : { draft: false }); return { ok: true }; }
      case 'jp_stats': { if (!window.JP || !JP.ready) return { ok: false, error: 'Japanese packs not loaded yet — open the module once' }; return { ok: true, ...JP.stats(), summary: JP.summary() }; }
      case 'jp_start_speaking': { if (!window.JP || !JP.ready) return { ok: false, error: 'open the Japanese module once first' }; const cat = input.scenario === 'free' ? null : input.scenario; const phrases = [...JP.items().values()].filter(x => x.type === 'phrase' && (!cat || x.p.cat === cat)).slice(0, 10).map(x => ({ ja: x.p.ja, romaji: x.p.romaji, en: x.p.en })); return { ok: true, scenario: input.scenario, rules: 'Speak only at his level (see Japanese context line); romaji in brackets after each Japanese line; corrections in [brackets]; English only if he asks; you play the other person.', phrases }; }
      case 'add_module': {
        const HUES = ['#F472B6', '#38BDF8', '#A3E635', '#FBBF24', '#C084FC', '#34D399', '#FB7185', '#60A5FA'];
        const used = new Set(Store.list('modules').map(m => (m.colour || '').toUpperCase()));
        const r = Store.insert('modules', { name: input.name, code: input.code || null, kind: input.kind || 'course', colour: HUES.find(h => !used.has(h)) || HUES[0], lecturer: input.lecturer || null, credits: input.credits ?? null, notes: input.notes || null, links: [] });
        return { ok: true, id: r.id, hub: '#/module/' + r.id };
      }
  ...
  const TOOL_LABEL = { add_module: 'Created module', jp_stats: 'Checked Japanese', jp_start_speaking: 'Speaking scenario', set_module_grade: 'Grade saved', set_assessment_mark: 'Mark saved', add_module_review: 'Reflection saved', confirm_module_review: 'Reflection updated', remember_fact: 'Remembered', forget_fact: 'Forgot', add_time_off: 'Time off to book', update_time_off: 'Time off updated', add_shift: 'Added shift', add_event: 'Added event', add_task: 'Added task', complete_task: 'Completed task', update_item: 'Updated', delete_item: 'Deleted', add_note: 'Saved note', add_watch: 'Watching', set_setting: 'Setting changed' };
  ...
  const Eden = {
    ...
    labelFor(a) {
      ...
      if (a.name === 'add_module') return `${L}: ${a.input.name}${a.input.kind === 'personal' ? ' (personal project)' : ''}`;
      return L;
    },
  };
  window.Eden = Eden;
